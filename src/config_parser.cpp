...
bool ConfigParser::ParseAbilities(const Json::Value &abilites)
{
    ...
    if (!abilites.isArray()) {
        cerr << "Error: abilites not array. at " << filePath_ << endl;
        return false;
    }
...
bool ConfigParser::ParseAbilitiy(const Json::Value &ability, bool &isMainAbility)
{
    ...
    if (abilityIconId_ <= 0) {
        cerr << "Error: iconId don't found in 'ability'. at " << filePath_ << endl;
        return false;
    }
    ...
    if (abilityLabelId_ <= 0) {
        cerr << "Error: labelId don't found in 'ability'. at "<< filePath_ << endl;
        return false;
    }
    isMainAbility = true;
    return true;
